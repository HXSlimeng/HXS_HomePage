  ...
  <div class="table-responsive m-3">
    <table class="table table-sm table-bordered caption-top">
      ...
      <tbody>
        ...
        <tr>
          <td>合计</td>
          <td v-for="(ctgr, ctgrI) in categorys">{{ summation[ctgr.prop] }}</td>
          <td>{{ summation.allin }}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
import { onMounted, ref, computed, toRef, Ref, unref } from "vue";
import "https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js";

const allData = ref<Array<{ [k: string]: number }>>([]);
const categorys = ref([
  {
    label: "吃",
    prop: "food",
...
const resetData: { [key: string]: number | null } = {
  date: null,
  food: null,
  clothes: null,
  entertainment: null,
...
const myModal = ref<{ hide: () => {}; show: () => {} }>();
onMounted(() => {
  //@ts-ignore
  myModal.value = new bootstrap.Modal("#dateModal", {
    keyboard: false,
  });
});

const compData = computed(() => {
  return allData.value.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
});

const selectedValues = ref<{ [key: string]: number | null }>({ ...resetData });
...
const summation = computed(() => {
  let initVal: typeof resetData = { date: null, food: null, clothes: null, entertainment: null, traffic: null, daily: null, others: null, allin: null };
